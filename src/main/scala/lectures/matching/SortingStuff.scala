...
object SortingStuff extends App {
  ...
  private def putStuffInRightBox(item: Stuff, stuffBox: StuffBox): StuffBox = item match {
    ...
    case it@Boots(brand, _) if Set("Adidas", "Converse").contains(brand) => stuffBox.copy(boots = it :: stuffBox.boots)
    case it@Book(_, isInteresting) if isInteresting => stuffBox.copy(books = it :: stuffBox.books)
